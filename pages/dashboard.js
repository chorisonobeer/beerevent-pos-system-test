...
export default function Dashboard() {
  ...
  const fetchDailyData = async (date) => {
    ...
    try {
      ...
      const data = await response.json();
      console.log('Fetched dashboard data:', data); // データ確認用
      setDailyData(data);
    } catch (error) {
...
<div className="bg-white rounded-lg shadow-md p-4 mb-4">
  ...
  <div className="grid grid-cols-2 gap-4">
    ...
    <div className="bg-slate-50 p-3 rounded-lg">
      ...
      <div className="text-xl font-bold text-slate-800">
        {formatCurrency(dailyData.totalSales || 0)}
      </div>
    </div>
  </div>
</div>
...
<div className="bg-white rounded-lg shadow-md p-4 mb-4">
  ...
  <div>
    <div className="flex justify-between text-sm text-slate-600 mb-1">
      <span>本日の売上: {formatCurrency(dailyData.totalSales || 0)}</span>
      <span>
...
<div className="mt-4">
  ...
  <div className="w-full bg-slate-200 rounded-full h-2.5">
    <div
      ...
      style={{ 
        width: `${Math.min(
          (dailyData.totalQuantitySold / dailyData.breakEvenPoint) * 100,
          100
        )}%`
      }}
    ></div>
...
<div className="mt-4">
  ...
  <div className="w-full bg-slate-200 rounded-full h-2.5">
    <div
      ...
      style={{ 
        width: `${Math.min(
          (dailyData.totalQuantitySold / dailyData.targetQuantity) * 100,
          100
        )}%`
      }}
    ></div>
